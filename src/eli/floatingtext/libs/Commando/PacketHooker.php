...
namespace eli\floatingtext\libs\Commando;

use eli\floatingtext\libs\Commando\exception\HookAlreadyRegistered;
use eli\floatingtext\libs\Commando\store\SoftEnumStore;
use eli\floatingtext\libs\Commando\traits\IArgumentable;
use eli\floatingtext\libs\simplepackethandler\SimplePacketHandler;
		...
		do {
			...
			foreach ($indexes as $k => $index) {
				...
				if (isset($param->enum) && $param->enum instanceof CommandHardEnum) {
					$enumName = $param->enum->getName();
					if ($enumName === "") {
						$enumName = "enum#" . spl_object_id($param->enum);
					}
					$param->enum = new CommandHardEnum($enumName, $param->enum->getValues());
				}
			}
